...
- (void)viewDidLoad {
    ...
    _scrollView.backgroundColor = RGBACOLOR(244, 244, 244, 1);
    ...
    CGFloat height = (_scrollView.frame.size.height - 3 * margin) / 4;
//    NSArray *imgUrls = @[
...
//                         ];
    ...
    UIButton *headBtn = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, kScreenHeight/3.9)];
    [headBtn setBackgroundImage:[UIImage imageNamed:@"bigHead.jpg"] forState:UIControlStateNormal];
//    [headBtn addTarget:self action:@selector(headAction:) forControlEvents:UIControlEventTouchUpInside];
    ...
    for (int i = 0; i < 1; i++) {
        for (int j = 0; j < 1; j++){
            ...
            if ((i==0&&j==1) || (i==1 && j==1)) {
                self.button = [[UIButton alloc] initWithFrame:CGRectMake(i * (margin + width), j * (margin + height) + kScreenHeight/3.9 + height/4, width - 2, height)];
            } else {
                self.button = [[UIButton alloc] initWithFrame:CGRectMake(i * (margin + width), j * (margin + height) + kScreenHeight/3.9, width - 2, height)];
            }

            self.button.tag = i * 2 + j;

            NSString *imageName = @"chengren";
//            [button sd_setBackgroundImageWithURL:[NSURL URLWithString:imgUrls[button.tag]] forState:UIControlStateNormal placeholderImage:[UIImage imageNamed:@"imageDownloadFail.png"]];
            ...
            CommodityInfoView *civ = [[CommodityInfoView alloc] initWithFrame:CGRectMake(i * (margin + width), CGRectGetMaxY(self.button.frame), width-2, height/3.5) tag:self.button.tag];
...
//  @{@"type":@"order", @"title":NSLocalizedString(@"em_chat_I_focus", @"I focus on"), @"order_title":[NSString stringWithFormat:@"%@%d",NSLocalizedString(@"order_number", @"Order number:"),456], @"desc":NSLocalizedString(@"em_example4_text", @"Cartier"), @"price":@"¥160000", @"img_url":@"http://o8ugkv090.bkt.clouddn.com/hd_four.png", @"item_url":@"http://www.baidu.com"}, nil];
    ...
    _infoArray = [NSArray arrayWithObjects:
                  @{@"type":@"track", @"title":@"平安成人意外保险", @"desc":@"", @"price":@"¥150", @"img_url":@"http://o8ugkv090.bkt.clouddn.com/hd_one.png", @"item_url":@"http://www.easemob.com"},
                   nil];   
}
